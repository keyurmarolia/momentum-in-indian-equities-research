...
- All 63 Python tests pass both in the working project and in a temporary clean source copy without the local market datasets. This tests source portability, not reproduction of the empirical dataset. Five frontend tests, TypeScript checks and the production build also pass.
...
The notebooks are the research record. Downloaded prices and generated dashboard data remain local; an unpopulated clean repository does not reproduce the saved empirical results without those inputs. Source-data redistribution rights require separate review before any public data release.
